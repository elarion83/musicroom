...
import { interactionObject, joinRoomNotifTextArray, playerRefObject, youtubeApiSearchObject, youtubeApiVideosParams } from "../services/utilsArray";
...
const Room = ({ t, currentUser, roomId, handleQuitRoom, setStickyDisplay }) => {
	...
	useEffect(() => {
        if(isVarExistNotEmpty(roomId)) {
            const initRoomFetchFirebase = async () => {
                ...
                initRoom(roomDatas, !firebaseRoom.exists(), roomRef);
            };
    ...
    async function initRoom(roomDatas, create = true, docRef = null) {
        if(create) {
    ...
    * */
    ...
    async function handleMediaEnd() {
        if(!mediaIndexExist(room.playlistUrls,playerIdPlayed+1)) {
            ...
            await setIdPlaying(playerIdPlayed+1);
        }
    }
...
// NEW FUNCTIONS FROM CHILD COMP
    ...
    useEffect(() => {
        if(loaded && spotifyPlayerReady && isVarExist(spotifyPlayerRef.current)) { 
            spotifyPlayerRef.current.setVolume(localVolume);
        }
    }, [localVolume,loaded, spotifyPlayerReady]);
    ...
    async function addMediaForAutoPlayByYoutubeId() {
        ...
        .then(async function(response) {
            if(!isEmpty(response.data.items)) {
                ...
                if(suggestMedia.platformId === actuallyPlayingMedia().platformId) {
    ...
    function actualPlayerRefIsPlaying(room) {
        return actuallyPlayingFromSpotify(room) ? spotifyPlayerIsPlaying : isVarExist(playerRef.current) ? playerRef.current.player.isPlaying : false;
    }
                                        ...
                                        <Grid container spacing={0} sx={{ bgcolor:'var(--grey-dark)'}} className={ isLayoutCompact(layoutDisplay) ? 'playerHide playerSection' : 'playerShow playerSection'}>
                                            ...
                                            <Grid item className='playerContainer' sm={4} xs={12} sx={{ pl:0,ml:0, pt: 0, position:'relative'}}>
                                                ...
                                                {isVarExist(actuallyPlayingMedia()) && 
                                                    <>
                                                        {!isFromSpotify(actuallyPlayingMedia()) ? 
                                                            (
                                                                <>
                                                                    <ReactPlayer 
                                                                        ref={playerRef}
                                                                ...
                                                                <>
                                                                    ...
                                                                    <Grid className="react-player" >
                                                                        <img style={{marginLeft:'auto', display:'block',marginRight:'auto',maxHeight:'240px'}} src={actuallyPlayingMedia().visuel} />
                                            ...
                                            <Grid item sm={8} xs={12} sx={{ padding:0,pl:0,ml:0, mb: 0,pt:0,height:'100%', color:'white' }} className={`player_right_side_container`}>
                                                ...
                                                <Grid item sm={12} sx={{ padding:0,pl:1.5,ml:0, mb: 0 , mt:1, fill:'#f0f1f0'}}>
                                                    ...
                                                    <Grid item sm={12} md={12} >                          
                                                        ...
                                                        {(spotifyPlayerReady && isVarExist(spotifyPlayerRef.current) && isFromSpotify(actuallyPlayingMedia())) && 
                                                            <Typography sx={{ fontSize: '10px', ml:0, mb: 1, color:'var(--grey-inspired)'}} className='fontFamilyNunito'>
                                                        ...
                                                        {(isLayoutDefault(layoutDisplay) || isLayoutFullScreen(layoutDisplay)) &&
                                                            <Box sx={{width:'100%'}} ref={el => animatedElementsRef.push(el)} className='animate__animated animate__fadeInUp animate__fast'>
                                                                ...
                                                                sx={{ display:'flex',justifyContent: 'space-between', padding:0,pt:1,ml:0,mr:1,pr:0, mb: 1.5 }}>
                                                                        ...
                                                                        {((isFromSpotify(actuallyPlayingMedia()) ? spotifyPlayerReady : youtubePlayerReady)) && <PlayerButtons 
                                                                            playerControlsShown={playerControlsShown}
                                                                            room={room}
                                                                            reSyncButtonShown={!(isActuallyAdmin) && (guestSynchroOrNot && (isFromSpotify(actuallyPlayingMedia()) ? spotifyPlayerReady : youtubePlayerReady))}
                                                                            ...
                                                                            playerIsPlaying={(isFromSpotify(actuallyPlayingMedia()) ? spotifyPlayerReady : youtubePlayerReady) && actualPlayerRefIsPlaying(room)}
                                                                            setIsPlaying={setIsPlaying}
