                ...
                {roomPlaylist.map(function (d, idx) {
                    return (
                        <Fade key={idx} in={true} xs={12} sx={{ width: '100%', p: 0, m: 0 }}>
                            <Grid item sx={{ width: '100%', p: 0, pl: 2, m: 0 }} className={`playlist_bloc ${(isFromSpotify(d)) ? !isSpotifyAvailable ? 'mediaUnavailable' : '' : 'mediaAvailable'}`}>

                                <ListItemButton
                                    onClick={e => handleChangeIdShownInDrawer(idx)}
                                    sx={{
                                        alignItems: 'flex-start', position: 'relative', width: '100%', pl: 2, m: 0, pr: '35px', backgroundColor: 'var(--main-bg-color)', borderBottom: '2px solid var(--border-color)', color: 'var(--white)', "&.Mui-selected": {
